<template>
    ...
    <ion-router-outlet :id="contentId"></ion-router-outlet>
      ...
      <ion-menu side="end" :content-id="contentId" menu-id = "learningIds" type="overlay">
      <ion-content>
      <ion-list>
        <ion-item v-for="(item) in learningContents" :key="item.id">
          <ion-label>
            <h2>{{ item.id }}</h2>
...
<ion-menu-toggle auto-hide="false">
      ...
      <ion-content :id="contentId">
        ...
        <div v-for="(segment, index) in segments" :key="index" class="segment">
            ...
            <!-- <ion-icon aria-hidden="true" :icon="bookmark"></ion-icon>
            <ion-text class="note-properties-label" color="medium">標籤</ion-text> -->
            <div>
            <ion-badge v-for="tag in segment.tags" class="note-properties-badge">{{tag}}</ion-badge>
            </div>
            <!-- <ion-label position="stacked" >標籤</ion-label>
            <ion-input v-model="segment.tag" placeholder="输入标签" value="#伽利略"></ion-input> -->
            </ion-item>
            <ion-item>
          ...
          <!-- 匹配的学习内容 ID -->
          <ion-item>
            <ion-label  class="classification-label">學習內容編碼</ion-label>
            <ion-badge v-for="tag in segment.learningContentIds" class="note-properties-badge">{{tag}}</ion-badge>
            <!-- <ion-select multiple v-model="segment.learningContentIds">
              <ion-select-option v-for="item in learningContentOptions" :key="item.id" :value="item.id">
                {{ item.name }}
              </ion-select-option>
            </ion-select> -->
          </ion-item>
          <!-- 匹配的学习表现 ID -->
          <ion-item>
            <ion-label  class="classification-label">學習表現編碼</ion-label>
            <ion-badge v-for="tag in segment.learningPerformanceIds" class="note-properties-badge">{{tag}}</ion-badge>
            <!-- <ion-select multiple v-model="segment.learningPerformanceIds">
              <ion-select-option v-for="item in learningPerformanceOptions" :key="item.id" :value="item.id">
                {{ item.name }}
              </ion-select-option>
            </ion-select> -->
          </ion-item>
  ...
  {
    id: 'Ba-Ⅳ-1',
    description: '能量有不同形式，例如：動能、熱能、光能、電能、化學能等，而且彼此之間可以轉換。孤立系統的總能量會維持定值。',
  },
  {
    id: 'Ba-Ⅳ-2',
    description: '光合作用是將光能轉換成化學能；呼吸作用是將化學能轉換成熱能。',
  },
  {
    id: 'Ba-Ⅳ-3',
    description: '化學反應中的能量改變，常以吸熱或放熱的形式發生。',
  },
  {
    id: 'Ba-Ⅳ-4',
    description: '電池是化學能轉變成電能的裝置',
  },
  {
    id: 'Ba-Ⅳ-5',
    description: '力可以作功，作功可以改變物體的能量。',
  },
  {
    id: 'Ba-Ⅳ-6',
    description: '每單位時間對物體所做的功稱為功率。',
  },
  {
    id: 'Ba-Ⅳ-7',
    description: '物體的動能與位能之和稱為力學能，動能與位能可以互換',
  },
]);
...
function submitSelection() {
  console.log('選擇的學習內容編碼：', selectedIds.value);
  ...
  onMounted(() => {
    setTimeout(()=>{
        menuController.open("learningIds");
    }, 3000);
